"""Process specs describing how one exogenous input is excited.

Each spec is a small frozen dataclass: an AR(1) drift, a log random walk, a
stochastic-volatility regime, rare crisis episodes, a per-run climate draw, or
the government-spending stabilizer. Specs are self-contained (an
:class:`AR1Spec` carries its own persistence ``phi``) and dt-aware: annual
``phi``/``sigma`` are discretized to sub-annual steps by :func:`discretize_ar1`
so the annual autocorrelation and stationary variance are invariant to the
timestep.
"""
...
class RandomWalkSpec:
    ...
    Used for ``Nfe`` (full-employment labour force): a level rather than a rate,
    so it drifts multiplicatively as ``base * exp(logdev)`` with ``logdev`` a
    bounded random walk.
    """
...
class CrisisSpec:
    ...
    * **severity** -- a common scale on every impulse, drawn from
      ``severity_range``;
    * **character** -- every episode carries the demand/credit ``impulses`` (an
      NPL jump, wider loan spread, collapsing investment spirits and sales
      expectations -- a credit crunch), and with probability ``financial_prob``
      also the ``financial_impulses`` (a collapse in equity portfolio preference
      ``lambda40`` that crashes the equity price and destroys wealth).

    The fiscal stabilizer (see :class:`GovSpendingSpec`) recovers most crises; a
    deep or long enough one can still tip the economy past its stable corridor,
    which surfaces as a truncated run rather than a bug.
    """

    prob: float  #: annual onset probability (converted to a per-step hazard by dt)
    impulses: Mapping[str, float]  #: demand/credit level shock per input at onset
    ...
    financial_prob: float = 0.0  #: chance an onset also triggers a financial crash
    #: equity-crash level shocks added when the financial component fires
    financial_impulses: Mapping[str, float] = field(default_factory=dict)

    @property
    def names(self) -> tuple[str, ...]:
        """Every input a crisis can shock (demand/credit and financial), stable order."""
